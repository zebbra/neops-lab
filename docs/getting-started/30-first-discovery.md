...
description: Run the discovery workflow, watch it execute, and see the devices with their interfaces appear in the CMS — then swap parameter files to change how discovery is targeted.
tags: [tutorial]
---

# Your first discovery

*One command turns the running containers into `Device` entities with their `Interface` rows.*

This page follows the default `wan-and-fabric` scenario, so the numbers below are its 15 devices. Another scenario discovers its own, from its own parameter files under `scenarios/<name>/workflow-execution-parameters/` — see [Scenarios](../30-scenarios/index.md).
...
2. `docker compose exec -T worker python3 lab/wait_devices` — confirm every device in the scenario's `topology.json` accepts SSH.
...
`make local-lab-discover` takes a `DISCOVER_PARAMS` override, so the same workflow can be pointed at four different shapes of input. The first three are generated by `make generate` for every scenario; the mixed file is hand-written and only exists where mixing platforms has something to express — `frr-only` has no such file, because every device there runs the same NOS.
...
!!! note "`wait_devices` always waits for the *known* devices"
    Even when you point discovery at the `/24`, the readiness poll uses the
    scenario's `topology.json`, not the parameter file. A subnet target also
    contains unused addresses, and waiting on those would always time out.
